...
{    
    ...
    {
        ui->pushHarrisCorner->setEnabled(_q_pFormFocused && _q_pFormFocused->Atrb() == "GRAY" && _q_pFormFocused->ImageGray().Address());
    }
...
void MainFrame::on_pushSepiaTone_clicked()
{
    ...
    for(unsigned int i = 0; i<arHSV.Row(); i++)
        for(unsigned int j = 0; i<arHSV.Col(); j++)
        {
            arHSV[i][j].h = dHue;
            arHSV[i][j].s = dSat;
        }
...
void MainFrame::on_pushGaussianSmoothing_clicked()
{
    ...
    // 가우시안 노이즈
    double dSigma = ui->editSigmaFilterring->text().toDouble();
...
void MainFrame::on_pushHistogramEQ_clicked()
{
    ...
    // 파일에 저장
    ofstream open("./output/hist_after.csv");
    for(int i=0; i<oHisto.Dim(); i++)
        fout << oHisto[i] << ',';
    fout.close();
...
void MainFrame::on_pushHarrisCorner_clicked()
{
    ...
    // while(oCorner.Pop(oCornerPixel) != -1) // 오류
    //     q_pForm->DrawEllipse(QPoint(oCornerPixel.u, oCornerPixel.v),1,1,QColor(255,0,0),2);
...
void MainFrame::on_pushCircleLocalization_clicked()
{
    ...
    KPGM(igMain).Save("./output/edge4Hough.pgm"); //확인용
...
void MainFrame::on_pushOpticalFlow_clicked()
{
    ...
    QFileDialog::Options q_Options = QFileDialog::DontResolveSymlinks |
                                     QFileDialog::DontUseNativeDialog;
    QStringList _stlFile = QFileDialog::getOpenFileNames(this, tr("Select Images"), "./data",
...
void MainFrame::on_pushOpticalFlowRun_clicked()
{
    ...
    // 파일 리스트를 지역 변수로 선언하고 초기화
    static QStringList stlFile;

    KOpticalFlow* _opOpticalFlow;

    // 영상 읽어들이기
    if (++nNo >= stlFile.length() || q_oImage.load(stlFile.value(nNo)) == false)
    {
        nNo = 0;
        delete _opOpticalFlow;
        _opOpticalFlow = 0;
    ...
    // Optical Flow 계산
    KOpticalFlow localOpticalFlow;
    KArray<KPOINTF>& arFlow = *(KArray<KPOINTF>*)_opOpticalFlow->Run((void*)&igImg);

    // Optical Flow 결과를 출력창에 표시
    ImageForm* _q_pFormFlow = nullptr;
    _q_pFormFlow = new ImageForm(igImg.GrayToRGB(), "Optical Flow", this);
    _lImageForm.push_back(_q_pFormFlow);

    _q_pFormFlow->Update(igImg.GrayToRGB());

    // 출력찾에 optical flow 표시
    KOPTICALFLOW_INFO   oInfo = *(KOPTICALFLOW_INFO*)_opOpticalFlow->Info();
    double              dScale = 3.0;
    int                 nStep = ui->editPeridFlow->text().toInt() * oInfo.nStride;

    for (int nVo = oInfo.nSy; nVo < oInfo.nEy; nVo += nStep)
    {
        for (int nUo = oInfo.nSx; nUo < oInfo.nEx; nUo += nStep)
        {
            // Optical Flow의 크기 및 방향 계산
